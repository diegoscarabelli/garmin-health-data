...
**Retroactively-editable data:** `MENSTRUAL_CYCLE_DAY` is re-fetched over a trailing 90-day window on every run, independent of the auto-detected start (the start is only ever extended backward, so explicit full-history ranges are unaffected). When a period start is edited after the fact in Garmin Connect, Garmin recomputes `day_in_cycle` for every following day; re-fetching a fixed recent window keeps those already-stored rows in sync via the upsert instead of leaving them stale. Before that per-day fan-out runs, a single probe of the menstrual calendar (summary) endpoint over the same window checks whether any cycle overlaps it; if none does (e.g. an account that doesn't track menstrual data), all ~90 day-view calls are skipped that run, since the dayview endpoint only returns data for dates inside a cycle window. The probe fails open, so a transient error on it falls back to running the full window.
...
`supplemental_activity_metric` is a flexible key-value table (`activity_id`, `metric`, `value`) for numeric fields that don't warrant a dedicated column. Most rows come from Activities List / activity-detail API fields left over after the other extraction methods run. A FIT-only subset also lands here, read directly from the FIT `session` message rather than the API, since Garmin's API never exposes them: advanced cycling pedal dynamics (`avg_left_torque_effectiveness`, `avg_right_torque_effectiveness`, `avg_left_pedal_smoothness`, `avg_right_pedal_smoothness`, `avg_left_pco`, `avg_right_pco`), power/cadence while seated vs. standing (`avg_power_position_seated` / `avg_power_position_standing`, `max_power_position_seated` / `max_power_position_standing`, `avg_cadence_position_seated` / `avg_cadence_position_standing`, `max_cadence_position_seated` / `max_cadence_position_standing`), and mechanical work / subjective effort (`total_work`, `avg_vam`, `threshold_power`, `time_standing`, `stand_count`, `workout_feel` and `workout_rpe`, stored raw on Garmin's 0-100 scale).
...
The field splits into two groups: tools that build a structured local SQLite
database for programmatic or LLM querying, and tools that take a different
approach entirely — flat-file export or a GUI-first archive. Comparing them
side by side in a single table obscured more than it clarified, so they're
split below; garmin-health-data anchors both for reference.
